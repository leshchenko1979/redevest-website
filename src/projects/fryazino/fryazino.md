...
|-----|---------------------|---------------------------|---------------------------|
| №1 (лучший вход) | 84 800 | +12,0% | +16,7% |
| №2 | 87 500 | +8,6% | +13,1% |
...
- **Предмет обеспечения (кадастр/ЕГРН).** База сделки — реальный объект: склад и земельный участок. Участок в собственности, межевание выполнено, правовой статус подтверждается документами. Точные кадастровые номера и выписки ЕГРН выдаются в боте по конкретному лоту после запроса инвестора.
- **Запас по стоимости актива относительно цены входа.** Расчет соотношения цены входа к стоимости актива и запас по покрытию дается индивидуально по выбранной доле: с привязкой к параметрам входа, документам по объекту и структуре конкретной сделки.
